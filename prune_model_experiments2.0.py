...
save_dir = "/home/jonna/hyperparameters-under-pruning/experiments2.0/cifar10/resnet56/finetuning/magnitude_20/results"
...
for root, dirs, files in os.walk(
    "/home/jonna/hyperparameter_sensitivity_pruning/experiments/cifar10/resnet56/base_model"
):
    for file in files:
        if file.endswith("checkpoint_final.th"):
            name = root.split("/")[-1]
            savedir = save_dir + f"/{name}"
            if Path(f"{savedir}/resnet56_magnitude_20.th").is_file():
                break
            ...
            pruning_method = PruningMethod(
                scorer, [tactic], target=Target(Flops(), fraction=0.2)
            )
            ...
            torch.save(
                model,
                f"{savedir}/resnet56_magnitude_20.th",
            )